...
/**
 * 言語設定が英語以外の場合にのみ、翻訳された文字列を取得する。英語設定の場合は空文字列を返す。
 * メインのキャプションには常に英語、説明文に翻訳を表示したい時に使う。
 * @param message 国際化文字列を格納した文字列リソース。
 * @param values
 * @returns
 */
export function getNonEnglish(message: I18NText, values: Record<string, string> = {}): string
{
	return localeKey() !== 'en' ? t(message, values) : '';
}
...
export function getOsDependentExplorerAppName(forceEnglish: boolean = false): string
{
	...
	{
		// Windows
		key = COMMON_TEXTS.windowsExplorer;
	}
	else if (process.platform === 'darwin')
	{
		// Mac
		key = COMMON_TEXTS.macFinder;
	}
	else
	{
		// Linux
		key = COMMON_TEXTS.linuxFileManager;
	}

	return forceEnglish ? en(key) : t(key);
}
